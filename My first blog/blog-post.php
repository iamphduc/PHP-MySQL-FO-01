...
// Lay id bai blog
$id = $_GET['id'];
$cate_name = $_GET['category_id'];

// Cau lenh truy van bai blog
$query_blog = "SELECT p.*, c.title AS 'category', a.name FROM posts p LEFT JOIN categories c ON p.category_id = c.id LEFT JOIN authors a ON p.author_id = a.id WHERE p.status = 1 AND p.id =".$id;
$result_blog = $conn -> query($query_blog);

$blog = $result_blog -> fetch_assoc();

// 5 bai MOST READ moi nhat
$query_mostRead = "SELECT p.*, c.title AS 'category' FROM posts p LEFT JOIN categories c ON p.category_id = c.id WHERE p.status = 1 AND p.category_id = ".$cate_name." ORDER BY p.created_at desc limit 5";
					...
					<div class="col-md-8">
						<div class="section-row sticky-container">
							<div class="main-post">
								...
								<p><?= $blog['contain']?></p>
								<p><strong><em>Đăng bởi: <?= $blog['name']?></em></strong></p>
							</div>
